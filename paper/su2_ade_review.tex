...
09-23 新增完整主支纯符号程序、$2T$ 十二类实际体积证明、
$2O$ 的 84 类和 $2I$ 的 563 类完整解析参数，以及两群不能全部有理化的严格反例。
第二轮的极对偶公式给出全部类型不含 $\Li_2$ 的精确体积和退化输入的初等相位，并完成 D 表对任意 $n$ 的证明。
旧计数代表与数值记录仅作为历史材料保留，不参与新路线。
\end{abstract}
...
\]
\input{su2_group_edge_formula.tex}
\input{su2_symbolic_ade.tex}
\input{su2_polar_dual.tex}
\input{su2_exact_catalogue.tex}
...
有限穷举本身不构成任意 $n$ 的证明。完整证明见定理 \ref{thm:dic-all-n}：该表在正规化子 $K$ 上与 $n$ 无关，16 个分支的 $\delta q$ 经形式地板代数恒为整数，所以对全部 $n$ 闭合并归一；再经透镜配对、Wigner 定理和 $H^4(BK;\Z)\cong\Z$，证明它与几何 E/F/T 代表同类，类阶为 $4n$。另外对 $n\le16$ 的显式基本闭链做了精确核对。

\input{su2_e_explicit.tex}
\input{su2_e_algebraic.tex}

\section{09-22 历史复现记录}\label{sec:ade-validation}

以下记录属于 09-22，不作为 09-23 解析/几何新路线的证明。
当时新增的精确计数接口与验证记录为：
\begin{itemize}
\item 当时 39 项单元测试通过，覆盖原几何接口及新增计数、链边界、
正依赖、射线缩放、字段约化和输入验证；
...
\item \path{docs/review-0922/SU2_HUMAN_REVIEW_RESPONSE.md}：09-22 审计及可复制公式。
\end{itemize}
09-22 还修复 $a^rb$ 坐标的末分量符号（应为 $-\sin(\pi r/n)$）、
...
make completepdf
\end{verbatim}
该阶段 checkout 无 09-21 命名资料或原源 A 附件，沿用已有 A 负号约定。
当时已给 E 型有限几何公式及代数计数公式、余边界转换和类阶证明，
但未导出专属短表；任意 $n$ 的 D 表逐分支证明也未补齐。
09-23 的新范围按用户要求聚焦 E 型；当前十二类、84 类、563 类及真实化简边界
以第 \ref{sec:symbolic-ade}--\ref{sec:oi-exact} 节为准，不以旧记录替代。
\end{sloppypar}
