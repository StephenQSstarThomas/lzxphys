\section{任务 3：E 型群元素的完整相位公式}\label{sec:E-explicit}
...
\ell_{\nu,ij}=\arccos\frac{v_{\nu i}\cdot v_{\nu j}}
{\|v_{\nu i}\|\|v_{\nu j}\|},\qquad
(ij)=(23,13,03,01,02,12).
\]
由式 \eqref{eq:explicit-edge-B} 中已全部展开的函数得到
\begin{equation}
...
\quad m=6,7,8.}
\end{equation}
所有符号和求和项均已由群元定义。它是一个完整的几何代表，采用通用有限分支；
尚未化成类似 D 型八行表的 E 型专属短表，不能把这项进一步化简写成已完成。
...
然后 Epa--Ganter 定理 1.1\cite{epa} 才给出
\[
