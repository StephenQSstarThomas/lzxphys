...
本节完成的是：一般位置的完整解析主程序、$2T$ 十二类的实际有理体积、
$2O$ 的 84 类和 $2I$ 的 563 类完整非退化参数表，以及原 E/F/T
退化链的有限解析接口。本节的 CAS 化简只完成了一部分；第 \ref{sec:polar-dual} 节用极对偶给出全部类型的初等闭式，从而取代了它。要求所有类型都是有理体积，已被下文反例排除。
...
$N_{F_4}$ 为下文严格证明的 $F_4$ 室数（与第 \ref{sec:complete-gram-table} 节的锚定重数 $m$ 无关）。
表中体积是无向体积；有序输入的有向体积再乘 $\operatorname{sgn}D$。
\begin{center}\small
\begin{tabular}{@{}rrrrrr|rrc@{}}
\toprule
\multicolumn{6}{c|}{$K$}&集合重数&$N_{F_4}$&$\Vol/\pi^2$\\\midrule
...
\begin{align}
N_{F_4}(V)&=\#\{p\in P:(V^{-1}p)_j>0\text{ 对所有 }j\},\\
V_{\rm or}&=\operatorname{sgn}(\det V)\frac{N_{F_4}(V)\pi^2}{576},\\
\omega_k&=\exp\left[-\frac{ik\pi}{576}\operatorname{sgn}(\det V)N_{F_4}(V)\right].
\end{align}
...
锥点通常不属于 E 群，不能强行归入十二、84 或 563 类型表；但在保持原锥点的前提下，式 \eqref{eq:degenerate-elementary} 把退化相位化成了初等函数。
...
预算内未完成不等同于不可化简。这张表只是历史尝试记录：全部 659 类的初等闭式由第 \ref{sec:polar-dual} 节给出，不依赖 CAS 预算。
